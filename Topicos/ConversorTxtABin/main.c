...
int generarEmpleadosTxt(const char *txt)
{
	...
	for (int i = 0; i < 10; i++) {
		fprintf(archBin, "%d|%s|%c|%d/%d/%d|%.2f\n", vEmpls[i].dni,
			vEmpls[i].apyn, vEmpls[i].sexo, vEmpls[i].fIngr.dia,
			vEmpls[i].fIngr.mes, vEmpls[i].fIngr.anio,
			vEmpls[i].sueldo);
	}
	fclose(archBin);
	return TODO_OK;
...
int mostrarEmpleadosBin(const char *nomArchBin)
{
	...
	while (fread(&empl, sizeof(Empleado), 1, archBin) == 1) {
		printf("%d | %s | %c | %02d/%02d/%04d | %.2f\n", empl.dni,
		       empl.apyn, empl.sexo, empl.fIngr.dia, empl.fIngr.mes,
		       empl.fIngr.anio, empl.sueldo);
	}
