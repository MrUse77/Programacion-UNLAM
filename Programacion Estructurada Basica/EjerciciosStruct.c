...
void ej1(){
    ...
    do{
        ...
        }while(productos[i].code<10000 || productos[i].code>99999||Buscar(productos, productos[i].code, j)!=i );
        ...
        productos[i].total=productos[i].price* (float)productos[i].unidadesVendidas;
        i++;

    }while(i!=j && strcmpi(productos[i-1].description, fin)!=0);
    do{
        int codigoProducto = 0, aux=0;
        do {
            printf("Ingrese Codigo de producto");
            scanf("%d", &codigo);
        }while((codigo<10000 || codigo>99999)&&Buscar(productos, codigo, j)==-1);
    ...
    do {
        ...
        i = BuscarMedicamentos2(medicamento, codigo, 50);
            if (i == -1) {
                ...
                printf("%d \t %f \t %d\n", medicamento[i].code, medicamento[i].price, medicamento[i].stock);
            }
...
int INGRESO3(alumno alumnos[]){
    ...
    do {
        do {
            ...
            //con 0 termina
        }while(((alumnos[i].dni<10000000 || alumnos[i].dni>99999999)&&alumnos[i].dni!=0) || BuscarAlumno(alumnos, alumnos[i].dni, i+1)!=i );
...
void RESULTADO3(alumno alumnos[],int cant){
    ...
    for(i=0;i<j;i++){
        printf("%d \t %s \t %d \t %d \t %.2f\n", aprobados[i].dni, aprobados[i].name, aprobados[i].nota1, aprobados[i].nota2, aprobados[i].promedio);
    }
    printf("REPROBADOS\n");
    printf("\t DNI \t NOMBRE \t NOTA 1 \t NOTA 2 \t PROMEDIO\n");
    for(i=0;i<k;i++){
        printf("%d \t %s \t %d \t %d \t %.2f\n", reprobados[i].dni, reprobados[i].name, reprobados[i].nota1, reprobados[i].nota2, reprobados[i].promedio);
    }
}
void INFORME_PROM(alumno alumnos[],int cant){
    ...
    printf("DNI \t NOMBRE \t NOTA 1 \t NOTA 2 \t PROMEDIO\n");
    for(i=0;i<j;i++){
        printf("%d \t %s \t %d \t %d \t %.2f\n", promocionado[i].dni, promocionado[i].name, promocionado[i].nota1, promocionado[i].nota2, promocionado[i].promedio);
    }
}
