...
void ej1(){
    Alumno alumno[5]={{44937522,"Agustin Doricich",10,10},{24387232,"Mash Burndead",1,1},{1238912,"Maximiliano Barata Vaz",9,10},{12328,"El Gabi",7,6},{129378291,"lolero puto",2,1}};
    FILE *f;
    int i;
...
void ej2(){
    ...
    while(!feof(f)){
        if((alumno[i].nota1>=4 && alumno[i].nota2>=4)&& (alumno[i].nota1 < 7||alumno[i].nota2<7)){
            printf("%d\n",alumno[i].dni);
    ...
    while(!feof(prom)){
        printf("%d \t %s \t %d \t %d \t %.2f\n",alumno[i].dni,alumno[i].nombre,alumno[i].nota1,alumno[i].nota2,alumno[i].promedio);
        i++;
        fread(&alumno[i],sizeof(Alumno),1,prom);
    }
    printf("APROBADOS\n");
...
void ej3(){
    ...
    printf("Desde: ");scanf("%d",&anio1);
    printf("Hasta: ");scanf("%d",&anio2);
    for(int i=0;i<12;i++){
...
void crearVentas3(){
    ...
    for(int i=0;i<100;i++){
        ...
        v->importe= 100.00 + (float)rand() / ((float)RAND_MAX / (1000.00 - 100.00));
