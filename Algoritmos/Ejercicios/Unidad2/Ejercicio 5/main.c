...
int colocarProdEnPila_TXT(t_Pila *p);
...
void verProd(t_Pila *p)
{
	Prod prod;
	verTope(p, &prod, sizeof(Prod));
	printProd(&prod);
}
...
void menu(t_Pila *p)
{
	...
	while (opcion != 0) {
		...
		case 0:
			break;
		case 1:
			if (pilaLlena(p, sizeof(Prod) == TRUE)) {
...
int save(t_Pila *p)
{
	...
	while (pilaVacia(p) != TRUE) {
		desapilar(p, &prod, sizeof(Prod));
